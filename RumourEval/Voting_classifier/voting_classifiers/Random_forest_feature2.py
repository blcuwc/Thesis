...
def preprocess_probabilities(path_branch, path_distill, dataset):
    ...
    train_id_pro = { ID: [branch_train_id_pro[ID][0], [ branch_train_id_pro[ID][1][i] + probabilities[i] for i in range(4) ]] for ID, probabilities in zip(distill_train_pro_dict["ID"], distill_train_pro_dict["Probabilities"]) if ID in train_id_label}
    ...
    dev_id_pro = { ID:[branch_dev_id_pro[ID][0], [ branch_dev_id_pro[ID][1][i] + probabilities[i] for i in range(4) ]] for ID, probabilities in zip(distill_dev_pro_dict["ID"], distill_dev_pro_dict["Probabilities"]) if ID in dev_id_label}
    ...
    test_id_pro = { ID:[branch_test_id_pro[ID][0], [ branch_test_id_pro[ID][1][i] + probabilities[i] for i in range(4) ]] for ID, probabilities in zip(distill_test_pro_dict["ID"], distill_test_pro_dict["Probabilities"]) if ID in test_id_label}
...
def voting_classifier(input_data, fold_num):
    ...
    predicted_labels = clf.predict(np.array(test_X))
